...
2. **Service** `CommercialMetricsService`
   ...
   - Datas: usar `sold_at` ou `confirmed_at` — **fixir um campo** e documentar (recomendação: `confirmed_at` se existir; senão `updated_at` na confirmação / `sold_at`).
...
### A — Comercial
- [ ] Permission `metrics.view`
- [ ] `CommercialMetricsService` + endpoint
- [ ] Feature tests (isolation, draft excluded)
- [ ] Docs: campo de data da receita
...
## 10. Decisão de produto pendente (registrar ao implementar)

1. Data canônica da sale para métricas: `confirmed_at` vs `sold_at`.  
2. Conversão: lifetime vs só vendas no mesmo período do cadastro.  
3. Incluir `initial_consultation_amount` na “receita total” do dashboard ou card separado (recomendação: **card separado**, não somar com Sale para não misturar avaliação com procedimento).  
4. Canal de alerta low-stock: email vs WhatsApp.

Quando for implementar, abrir a **Onda A** como Phase 10a no roadmap e ir avançando onda a onda.
