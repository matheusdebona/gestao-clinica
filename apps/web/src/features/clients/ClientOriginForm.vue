...
import { clientOriginFormSchema } from '@/features/clients/schema'
import type { ClientOrigin, ClientOriginPayload } from '@/types/client'
...
const emit = defineEmits<{
  submit: [payload: ClientOriginPayload]
  cancel: []
}>()

const { defineField, handleSubmit, errors, setErrors, resetForm } = useForm({
  validationSchema: clientOriginFormSchema,
...
<template>
  <form class="flex flex-col gap-4" @submit.prevent="onSubmit">
    <FormField label="Nome" :error="errors.name" html-for="origin-name">
      <template #default="{ invalid }">
        <Input id="origin-name" v-model="name" v-bind="nameAttrs" type="text" :invalid="invalid" />
      </template>
    </FormField>
